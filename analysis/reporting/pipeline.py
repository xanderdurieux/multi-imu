...
def _write_report_md(
    ...
    lines = [
        ...
        "",
        "### Parsed Stage Summary (`figures/parsed/`)",
        f"- {n_stage_figures.get('parsed', 0)} figures:",
        "  - `parsed_session_bar_chart.png` — recording durations and sample counts",
        "  - `parsed_interval_jitter.png` — inter-sample interval median and IQR per recording",
        "  - `parsed_packet_loss.png` — estimated BLE packet loss per recording",
        "  - `parsed_clock_drift.png` — Arduino clock drift (ppm) per recording",
        "",
        ...
        "  - `orientation_quality_overview.png` — per-section quality overview",
        "  - `orientation_selected_residuals.png` — selected gravity residual per section",
        "  - `orientation_method_residual_heatmap.png` — residuals per method and sensor",
